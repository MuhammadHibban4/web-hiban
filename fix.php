...
if ($buy_now) {
    ...
    // Tambahkan 'food_id' dan satu '?'
    $stmt = $pdo->prepare("INSERT INTO order_items (order_id, food_id, name, price, quantity, subtotal) VALUES (?,?,?,?,?,?)");
    
    // --- PERUBAHAN DI SINI (baris 29) ---
    // Tambahkan $buy_now['id'] (atau 'food_id' jika itu nama kuncinya di session)
    $stmt->execute([$order_id, $buy_now['id'], $buy_now['name'], $buy_now['price'], $buy_now['qty'], $buy_now['price'] * $buy_now['qty']]);
    unset($_SESSION['buy_now']);
} else {
    // --- PERUBAHAN DI SINI (baris 32) ---
    // Tambahkan 'food_id' dan satu '?'
    $stmt = $pdo->prepare("INSERT INTO order_items (order_id, food_id, name, price, quantity, subtotal) VALUES (?,?,?,?,?,?)");
    
    foreach ($cart as $it) {
        // --- PERUBAHAN DI SINI (baris 34) ---
        // Tambahkan $it['id'] (atau 'food_id' jika itu nama kuncinya di session)
